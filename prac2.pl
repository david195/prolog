...
color(Color,S):- partes(Pr,_,Color,_,_),prov_part(S,Pr,_).
ciudad(Ciudad,S):- partes(Pr,_,_,_,Ciudad),prov_part(S,Pr,_).
notP(Pr,S):- prov_part(S,Px,_),Pr\=Px,not(yesP(Pr,S)).
yesP(Pr,S):- prov_part(S,Pr,_).
